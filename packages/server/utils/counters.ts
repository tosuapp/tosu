...
export function buildSettings(res: http.ServerResponse) {
        ...
        .replace(
            ...
            [
                settingsItemHTMLv2
                    ...
                    .replace(
                        '{description}',
                        `Allow real-time calculation of performance points.<br>
                        Useful to turn off for tournaments or users who do not care about pp.`
                    )
                    .replace(
                        '{input-1}',
                        settingsSwitchHTML
    ...
    const serverGroup = settingsGroupHTML.replace('{header}', 'Server').replace(
        '{items}',
        [
                    ...
                    settingsItemHTMLv2
                        ...
                        .replace(
                            ...
                            settingsTextInputHTML
                                ...
                                .replace('{value}', config.serverIP.toString())
                        ),
                    settingsItemHTMLv2
                        .replace('{name}', 'Server Port')
                        .replace(
                            '{description}',
                            `The server port tosu will listen on.<br>
            ...
            settingsItemHTMLv2
                ...
                .replace(
                    '{description}',
                    `A comma-separated list of IP addresses that are allowed to connect to the server.<br>
                    Server IP is automatically added to the list. | Supports wildcard support (eg. 192.168.*.*)`
                )
                .replace(
                    '{input-3}',
                    settingsTextareaInputHTML
            ...
            [
                settingsItemHTMLv2
                    ...
                    .replace(
                        '{description}',
                        `
                Allow logging of additional verbose data to the console.<br>
                May cause performance issues: Please do not enable unless troubleshooting.
                `
                    )
                    .replace(
                        '{input-1}',
