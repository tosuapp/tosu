...
const keybind_div = document.querySelector(
    '[data-id="INGAME_OVERLAY_KEYBIND"]'
);
...
const settingsBuilder = {
    ...
    setup(props) {
        ...
        for (let i = 0; i < props.settings.length; i++) {
            ...
            if (setting.type == 'options') setting.options.push('');
        ...
        async function updateSettings(event) {
            ...
            try {
                ...
                for (let i = 0; i < _settings.length; i++) {
                    ...
                    if (setting.type == 'commands') {
                        ...
                        setting.options.forEach((r) => {
                            if (r.values)
                                r.values = r.values.filter(
                                    (r) => r !== '' && r != null
                                );
                        });
                        setting.value = (setting.value || []).filter(
                ...
                const request = await fetch(
                    `/api/counters/settings/${props.folderName}?update=yes`,
                    {
                        method: 'POST',
        ...
        function addCommand(event, ind, ind2) {
            ...
            const find = item.value
                .slice(0, item.value.length - 1)
                .find((r) => r[item.uniqueCheck] == value[item.uniqueCheck]);
            if (find) {
                displayNotification({
            ...
            const required = items.filter(
                (r) =>
                    r.required == true && r.name !== '' && r.type != 'checkbox'
            );
        ...
        function pushOption(event, ind, index) {
            ...
            const find = item.options
                .slice(0, ind)
                .find((r) => r.toLowerCase() == value.toLowerCase());
            if (find) {
                highlight(event.target.parentNode.children[0]);
        ...
        function updateType(from, event, ind, ind2) {
            ...
            if (from === '') {
                ...
                if (value == 'checkbox')
                    item.value =
                        item.value === false || item.value === true
                            ? item.value
                            : false;
        ...
        function deleteOption(ind, index, force) {
            if (settings.value[index].options[ind] != '' && force != true)
                return;
            if (force == true) {
        ...
        function deleteOptionValue(ind, ind2, index, force) {
            if (
                settings.value[index].options[ind].values[ind2] != '' &&
                force != true
            )
                return;
        ...
        function addSetting(event) {
            ...
            const isExists = settings.value
                .slice(1)
                .find((r) => r.uniqueID == item.uniqueID);
            if (isExists) {
                displayNotification({
                    element: event.target,
        ...
        function removeSetting(ind) {
            const confirmed = confirm(
                `Delete option «${settings.value[ind].uniqueID}»?`
            );
            if (!confirmed) return;
...
const showSettings = {
    ...
    setup(props) {
        ...
        for (let i = 0; i < props.settings.length; i++) {
            ...
            if (setting.type == 'commands') {
                ...
                setting.value.forEach((command, cmd_ind) => {
                    setting.options.forEach((option) => {
                        ...
                        const modified = value.find(
                            (r) =>
                                r[setting.uniqueCheck] ==
                                command[setting.uniqueCheck]
                        )?.[option.name];
                        if (original == modified) return;

                        setting.value[cmd_ind][option.name] =
                            modified ?? original;
                    });
                });

                value.forEach((command) => {
                    const find = setting.value.find(
                        (r) =>
                            r[setting.uniqueCheck] ==
                            command[setting.uniqueCheck]
                    );
            ...
            if (setting.type != 'commands') setting.value = value;
            settings.value.push(setting);
        }
        ...
        async function updateSettings(event) {
            ...
            try {
                ...
                const values = _settings.map((r) => ({
                    uniqueID: r.uniqueID,
                    value: r.value
                }));

                const request = await fetch(
                    `/api/counters/settings/${props.folderName}`,
                    {
                        method: 'POST',
...
function getSizeOfElement(div) {
    ...
    size.width =
        size.width +
        +styles.paddingLeft.replace('px', '') +
        +styles.paddingRight.replace('px', '') +
        +styles.marginLeft.replace('px', '') +
        +styles.marginRight.replace('px', '');
    size.height =
        size.height +
        +styles.paddingTop.replace('px', '') +
        +styles.paddingBottom.replace('px', '') +
        +styles.marginTop.replace('px', '') +
        +styles.marginBottom.replace('px', '');
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                loadingDiv.style.width =
                    loadingDiv.style.height = `${childrenSize.height}px`;
            }, 100);
        }, 10);
    }
}
...
async function downloadCounter(element, id, update) {
    ...
    const url = new URL(
        `http://${window.location.host}/api/counters/download/${folderName}`
    );
    ...
    if (update == true)
        text = `PP Counter updated: ${name} by ${author}<br>Refresh it in obs (if you have one added)`;
    ...
    if (div_element.classList.contains('result-item'))
        div_element.classList.add('downloaded');
    if (div_element.classList.contains('result-item'))
        div_element.classList.remove('updatable');
}
...
async function loadCounterSettings(overlay_name, to, no_modal) {
    ...
    if (no_modal == true) {
        window.counter_settings = createApp(showSettings, {
            ...
            values:
                typeof json?.values == 'object' && !Array.isArray(json?.values)
                    ? json.values
                    : {}
        });
...
async function startBuilderModal(element) {
    ...
    try {
        const download = await fetch(
            `${window.location.origin}/${folderName}/settings.json`
        );
...
window.addEventListener('click', (event) => {
    ...
    if (t?.classList.value.includes(' open-folder-button'))
        return openCounter(t);
    ...
    if (t?.id == 'devMode') {
        document
            .querySelectorAll('.settings-builder-button')
            .forEach((r) => r.classList.toggle('active'));
        return;
    }
});
...
if (available_overlays)
    available_overlays.innerHTML = `Available (${cachedAvailable})`;
...
window.onload = async () => {
    ...
    try {
        ...
        for (let i = 0; i < installed.length; i++) {
            ...
            const updatable =
                counter.attributes.getNamedItem('v')?.value != find.version;
        ...
        if (event.type === 'keydown') {
            ...
            if (!selected_keys.some((k) => k.key === formattedKey)) {
                ...
                selected_keys.sort((a, b) => {
                    ...
                    const isModifierA = modifierOrder.includes(
                        a.key.split('+')[0]
                    );
                    const isModifierB = modifierOrder.includes(
                        b.key.split('+')[0]
                    );
                    if (isModifierA !== isModifierB)
                        return isModifierA ? -1 : 1;
                    return a.key.localeCompare(b.key, undefined, {
                        numeric: true
                    });
                });
                keybindInput.value = selected_keys
                    .map((k) => k.key)
                    .join(' + ');
            }
        } else if (event.type === 'keyup') {
            selected_keys = selected_keys.filter(
                (k) => k.code !== event.keyCode
            );
