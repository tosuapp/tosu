    ...
    sendCommand(command, overlay_name, payload, amountOfRetries = 1) {
        ...
        try {
            ...
            if (amountOfRetries <= 3) {
                console.log(
                    `[COMMAND_ERROR] Attempt ${amountOfRetries}`,
                    error
                );
                setTimeout(() => {
                    that.sendCommand(
...
const app = createApp({
    setup() {
        ...
        const overlays = computed(
            () =>
                settings.value.profiles[settings.value.profile]?.overlays || []
        );
        ...
        function resizing(event) {
            ...
            if (event.ctrlKey) {
                const client_rect = element.getBoundingClientRect();
                const offset_x =
                    (event.clientX - start_x) * element.style.scale;
            ...
            if (resize_direction.top) {
                ...
                if (height >= 10 && top + height <= max_height.value) {
                    find.height = round_up(
                        Math.max(10, height / initial_scale)
                    );
        ...
        window.addEventListener('mousemove', (event) => {
            ...
            find.top = round_up(
                Math.max(
                    0,
                    Math.min(
                        max_height.value - element_height,
                        initial_top + offset_y
                    )
                )
            );
            find.left = round_up(
                Math.max(
                    0,
                    Math.min(
                        max_width.value - element_width,
                        initial_left + offset_x
                    )
                )
            );
        ...
        window.addEventListener('contextmenu', (event) => {
            if (event.target.classList.contains('overlay')) {
                ...
                setTimeout(() => {
                    ...
                    const toRight =
                        event.clientY + size.height > max_height.value;
                    const toTop = event.clientX + size.width > max_width.value;

                    context_overlay.value.y = toRight
                        ? event.clientY - size.height
                        : event.clientY;
                    context_overlay.value.x = toTop
            ...
            setTimeout(() => {
                ...
                context_empty.value.y = toRight
                    ? event.clientY - size.height
                    : event.clientY;
                context_empty.value.x = toTop
                    ? event.clientX - size.width
                    : event.clientX;
        ...
        function add_overlay(overlay) {
            ...
            settings.value.profiles[settings.value.profile].overlays.push({
                ...
                height: Math.min(
                    max_height.value - context_empty.value.y1,
                    overlay.resolution[1]
                ),
        ...
        function reset_overlay(index) {
            ...
            const find = available_overlays.value.find((r) =>
                item.url.endsWith(r.folderName)
            );
            if (!find) return;

            item.width = Math.min(
                max_width.value - item.left,
                find.resolution[0]
            );
            item.height = Math.min(
                max_height.value - item.top,
                find.resolution[1]
            );
        ...
        function remove_overlay(index) {
            settings.value.profiles[settings.value.profile].overlays.splice(
                index,
                1
            );
            context_overlay.value._ = false;
                ...
                if (command == 'getSettings') {
                    ...
                    if (Object.keys(message).length == 1 && message.overlays)
                        message = {
                            profile: 0,
                            profiles: [
                                { name: 'default', overlays: message.overlays }
                            ]
                        };
