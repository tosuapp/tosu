...
export class GamePlayData {
    ...
    HitKatu: number = 0;
    HitMiss: number = 0;
    HitMissPrev: number = 0;
    HitUR: number = 0.00;
    HitSB: number = 0;
    ComboPrev: number = 0;
    Combo: number = 0;
    PlayerHPSmooth: number = 0.00;
    PlayerHP: number = 0.00;
    Accuracy: number = 0.00;
    UnstableRate: number = 0;
    GradeCurrent: string = "";
    GradeExpected: string = "";
    Leaderboard?: Leaderboard;
    KeyOverlay: KeyOverlay = {} as KeyOverlay;
    ...
    async updateState() {
        const { process, bases, allTimesData, menuData } = this.services.getServices(["process", "bases", "allTimesData", "menuData"]);
        ...
        this.Mods = process.readInt(process.readInt(scoreBase + 0x1C) + 0xC) ^ process.readInt(process.readInt(scoreBase + 0x1C) + 0x8)
        ...
        this.PlayerHPSmooth = process.readDouble(process.readInt(gameplayBase + 0x40) + 0x14)
        ...
        // [Ruleset + 0x7C] + 0x24
        const leaderBoardAddr = leaderBoardBase > 0 ? process.readInt(leaderBoardBase + 0x24) : 0
        wLogger.debug(`leaderboardAddr = ${leaderBoardAddr.toString(16)} (${leaderBoardAddr})`)
        if (!this.Leaderboard) {
        ...
        if (this.MaxCombo > 0) {
            ...
            if ((this.Mods & OsuMods.DoubleTime) === OsuMods.DoubleTime || (this.Mods & OsuMods.Nightcore) === OsuMods.Nightcore) {
        ...
        const keyOverlayArrayAddr = process.readInt(process.readInt(process.readInt(rulesetAddr + 0xB0) + 0x10) + 0x4)
        this.KeyOverlay = this.getKeyOverlay(process, keyOverlayArrayAddr);

        const remaining = menuData.ObjectCount - this.Hit300 - this.Hit100 - this.Hit50 - this.HitMiss
    ...
    private getKeyOverlay(process: Process, keyOverlayArrayAddr: number) {
        return {
            // [Base + 0x8] + 0x1C
            K1Pressed: Boolean(process.readByte(process.readInt(keyOverlayArrayAddr + 0x8) + 0x1C)),
            // [Base + 0x8] + 0x14
            K1Count: process.readInt(process.readInt(keyOverlayArrayAddr + 0x8) + 0x14),
            // [Base + 0xC] + 0x1C
            K2Pressed: Boolean(process.readByte(process.readInt(keyOverlayArrayAddr + 0xC) + 0x1C)),
            // [Base + 0xC] + 0x14
            K2Count: process.readInt(process.readInt(keyOverlayArrayAddr + 0xC) + 0x14),
            // [Base + 0x10] + 0x1C
            M1Pressed: Boolean(process.readByte(process.readInt(keyOverlayArrayAddr + 0x10) + 0x1C)),
            // [Base + 0x10] + 0x14
            M1Count: process.readInt(process.readInt(keyOverlayArrayAddr + 0x10) + 0x14),
            // [Base + 0x14] + 0x1C
            M2Pressed: Boolean(process.readByte(process.readInt(keyOverlayArrayAddr + 0x14) + 0x1C)),
            // [Base + 0x14] + 0x14
    ...
    private updateStars() {
        const {
            settings, menuData, allTimesData, beatmapPpData
        } = this.services.getServices(["settings", "menuData", "allTimesData", "beatmapPpData"]);
        ...
        const scoreParams = {
            passedObjects: resolvePassedObjects(this.Mode, this.Hit300, this.Hit100, this.Hit50, this.HitMiss, this.HitKatu, this.HitGeki),
            combo: this.MaxCombo,
        ...
        const curPerformance = new Calculator(scoreParams)
            .performance(beatmap)

        const fcPerformance = new Calculator({
            ...scoreParams,
            passedObjects: resolvePassedObjects(this.Mode, this.Hit300 + this.HitMiss, this.Hit100, this.Hit50, 0, this.HitKatu, this.HitGeki),
            n300: this.Hit300 + this.HitMiss,
            nMisses: 0
        }).performance(beatmap);

        beatmapPpData.updateCurrentAttributes(curPerformance.difficulty.stars, curPerformance.pp)
