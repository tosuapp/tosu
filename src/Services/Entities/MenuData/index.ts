...
export class MenuData extends AbstractEntity {
	...
	async updateState() {
		...
		// [[Beatmap] + 0x18]
		this.Artist = process.readSharpString(process.readInt(beatmapAddr + 0x18));
		// // [[Beatmap] + 0x1C]
		this.ArtistOriginal = process.readSharpString(
			process.readInt(beatmapAddr + 0x1c)
		);
		// // [[Beatmap] + 0x24]
		this.Title = process.readSharpString(process.readInt(beatmapAddr + 0x24));
		// // [[Beatmap] + 0x28]
		this.TitleOriginal = process.readSharpString(process.readInt(beatmapAddr + 0x28));
		...
		//  [[Beatmap] + 0x64]
		this.AudioFilename = process.readSharpString(process.readInt(beatmapAddr + 0x64));
		// //  [[Beatmap] + 0x68]
		this.BackgroundFilename = process.readSharpString(
			process.readInt(beatmapAddr + 0x68)
		);
		//  [[Beatmap] + 0x78]
		this.Folder = process.readSharpString(process.readInt(beatmapAddr + 0x78));
		//  [[Beatmap] + 0x7C]
		this.Creator = process.readSharpString(process.readInt(beatmapAddr + 0x7c));
		//  [[Beatmap] + 0x80]
		this.Name = process.readSharpString(process.readInt(beatmapAddr + 0x80));
		//  [[Beatmap] + 0xB0]
		this.Difficulty = process.readSharpString(process.readInt(beatmapAddr + 0xb0));
		//  [Beatmap] + 0xCC
	...
	updateMP3Length() {
		...
		this.MP3Length = Math.round(
			process.readDouble(
				process.readPointer(bases.getBase('getAudioLengthAddr') + 0x7) + 0x4
			)
		);
	}
}
